...
$recu = (isset($_POST['recu'])) ? $_POST['recu'] : '';
$idsubcat = (isset($_POST['idsubcat'])) ? $_POST['idsubcat'] : '';

if ($idsubcat != 0) {
	$rela= mysqli_query($con, "SELECT DISTINCT id_ova FROM relacion WHERE id_subcat='$idsubcat'");
	if(mysqli_num_rows($rela) == 0){
				$html .= "<span class='glyphicon glyphicon-search' aria-hidden='true'></span> No se encontraron OVAS asociados a ese recurso</br></br>";
			}else{
				...
				$sql = mysqli_query($con, "SELECT * FROM objeto_ova WHERE id_objeto='$idova' AND id_tipo='$recu' AND ova_activo=1");
